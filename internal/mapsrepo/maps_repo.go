...
func NearestRoad(points []*latlng.LatLng) []*latlng.LatLng {
	...
	for _, coord := range points {
		convertedPoints = append(convertedPoints, fireToMapsLatLng(coord))
	}

	// Make request and handle errors
	request := &maps.NearestRoadRequest{Points: convertedPoints}
	snappedPoints, _, err := client.NearestRoad(context.Background(), request)
	if err != nil {
	...
	for _, snapPoint := range snappedPoints {
		roadPoints = append(roadPoints, mapsToFireLatLng(snapPoint.Location))
	}
